...
test.describe('User Journey — Add Intern', () => {
  ...
  /*
  OBSERVATION (Task 5.1):
  This journey test catches bugs that the Vitest unit test for AddInternForm cannot because Vitest runs in
  a simulated jsdom environment where components are isolated. It cannot verify:
  1. The integration between the form component and the global React context/state provider (e.g. if the context
     fails to append the new intern, the E2E test fails but the form unit test might pass).
  2. Real browser rendering behaviors, layouts, or cross-component side effects (like updating ScoreStats or
     filtering lists).
  */
...
test.describe('User Journey — Search and Filter', () => {
  ...
  /*
  OBSERVATION (Task 5.3):
  type() is more realistic than fill() for search inputs because it types character-by-character, firing
  all keydown, keyup, and keypress events sequentially. This mimics real human typing, making it perfect
  for verifying search filtering or autocomplete results that update reactively as the user types.
  */
  test('typing in search filters the intern list', async ({ page }) => {
    // All 4 interns visible initially
    await expect(page.getByRole('button', { name: 'Remove' })).toHaveCount(4);

    // Search input has label — use getByLabel or placeholder
...
test.describe('User Journey — Remove Intern', () => {
  ...
  /*
  OBSERVATION (Task 5.4):
  Using getByRole('heading', { name: 'Rahul' }).locator('..') is much better than .first() because it scopes
  the locator to Rahul's specific card. If the order of cards changes (e.g. sorted by score or date added),
  .first() would click the Remove button on whatever card is currently at the top of the list, whereas
  the heading-based scoping always finds Rahul's card correctly.
  */
...
test.describe('User Journey — Theme Toggle', () => {
  ...
  /*
  OBSERVATION (Task 5.5):
  Asserting the button label change is a valid test because it confirms the React state updated
  reactively. If the application added a CSS class to the body or root element, we would assert
  await expect(page.locator('body')).toHaveClass(/dark/) instead of relying solely on the button text.
  */
  ...
  test('clicking toggle switches to dark mode', async ({ page }) => {
    ...
    // Button label updates — theme applied
    await expect(
